...
Airports offer a network of daily meterological observation stations which are accessable via WeatherUnderground. [Open flights](   ) has global metadata for 7,184 airports, the majority of which report information to WeatherUnderground. 
...
unique(HydroData::ghcn_stations$START_YEAR)
```


### Finding Global Historic Climatology Network

The [GHCN (Global Historical Climatology Network)](  ) is an integrated database of daily climate summaries from land surface stations across the globe. `HydroData` offers access to data for 105,394 stations, offering over 137 unique measurments, the majority of which report information to WeatherUnderground. 

Some stations go back till 1836!

Here's a basic example for subseting all GHCN stations near UCSB.
