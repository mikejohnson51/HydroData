getDaymet <- function(AOI = NULL, tile.ids = NULL, year = NULL,
                      parameters = NULL, aggregate = NULL, crop.data = FALSE)
{
  ...
  all.years <- 1980:(as.numeric(format(Sys.Date(), "%Y")) - 1)

  if (!all(year %in% all.years)) {
    bad.year <- year[!(year %in% all.years)]
    stop("'", paste(bad.year, collapse = "', '"), "'", if (length(bad.year) > 1){
      paste(" are")
    } else {
      paste(" is")
  ...
  if(!is.null(tile.ids)) {
    ...
    A <- sp::spTransform(AOI$AOI, daymet_tiles@proj4string)
    tile.ids <- daymet_tiles[A, ]$TileID
  }

  daymet.data <- list()

  message("AOI covers ", length(tile.ids), " DAYMET tile(s)")
  ...
  for (k in seq_along(parameters))
  {
    ...
    for (i in seq_along(urls)){
      ...
      all.rast[[i]] = raster::raster(check$destfile)
    }
    ...
    {
      temp.rasters <- raster::brick(all.files[[i]])
      bounds <- sp::spTransform(AOI$AOI, temp.rasters@crs)
      if (crop.data)
      {
