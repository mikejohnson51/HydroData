getCLD = function(state = NULL, county = NULL, clip_unit = NULL, year = NULL, keep.boundary = FALSE, save = FALSE){
      ...
      urls = vector()
      tempd = tempdir()
      suppressWarnings( do.call(file.remove, list(list.files(tempd, full.names = T))))
      bad.years = vector()
      crops = list()
   ...
   for(j in 1:length(urls)){
    ...
    if(grepl('fault', URL)){
        for(i in 1997:2016){
          if(grepl('fault', getURL(paste0("https://nassgeodata.gmu.edu/axis2/services/CDLService/GetCDLFile?year=",i,"&bbox=",bb)))){
      ...
      }else{ download.file(url = as.character(xmlToDataFrame(URL)$text) , destfile = paste0(tempd,"/", year[j], ".tif"), quiet = TRUE)
    ...
    for(i in 1:length(all.files)){ crops[[paste0('year_', year[i])]] = raster(all.files[i]) }

    b = brick(crops)
    b = projectRaster(b, crs = AOI@proj4string, method = 'ngb')
    b = crop(b, AOI)
